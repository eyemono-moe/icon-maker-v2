...
| Initial client JavaScript graph | 969.59 kB | 316,458 bytes |
...
The bundle check follows the deterministic client route entry in Vite's
manifest, traverses manifest import identities for the initial graph, and
resolves `src/lib/svg-optimize.ts` as a required dynamic entry before measuring
its emitted file. This keeps the budget tied to the route actually bootstrapped
in the browser rather than to a chunk's display name or filename alone.
